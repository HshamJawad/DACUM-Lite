...
// ══════════════════════════════════════════════════════════════
//  LIST FORMATTING — bullets / numbering for section textareas
//  (parity with DACUM Live Pro)
//
//  toggleListFormat(inputId, mode)
//     mode = 'bullet' → prefixes every non-empty line with "• "
//     mode = 'number' → prefixes every non-empty line with "1. ", "2. " …
//
//  The action is a TOGGLE: if every non-empty line already carries the
//  requested marker, the markers are stripped instead. Switching from
//  one mode to the other replaces the markers in place. Empty lines are
//  preserved as-is and never numbered.
// ══════════════════════════════════════════════════════════════

// Any marker this tool may have written before — bullet glyphs or
// "1." / "1)" style numbers, with optional leading whitespace.
...
function lineHasMode(line, mode) {
    if (!line.trim()) return true;                       // blanks don't vote
    return mode === 'bullet'
        ? /^[ \t]*[•▪■◦*\-–]\s+/.test(line)
        : /^[ \t]*\d+[.)]\s+/.test(line);
}

export function toggleListFormat(inputId, mode) {
    const ta = document.getElementById(inputId);
    if (!ta) return;

    const lines = ta.value.split('\n');
    const hasContent = lines.some(l => l.trim());
    if (!hasContent) { ta.focus(); return; }

    // Already fully formatted in this mode? → strip (toggle off)
    const alreadyApplied = lines.every(l => lineHasMode(l, mode));

    let counter = 0;
    const out = lines.map(line => {
        const bare = stripListMarker(line);
        if (!bare.trim()) return '';                     // keep blank lines blank
        if (alreadyApplied) return bare;
        if (mode === 'bullet') return '• ' + bare;
        counter++;
        return counter + '. ' + bare;
    });
    ...
    // Let autosave / undo-redo / any other listener see the change.
    ...
    showStatus(
        t(alreadyApplied
            ? 'status.listCleared'
            : (mode === 'bullet' ? 'status.bulletsApplied' : 'status.numbersApplied')),
        'success'
    );
}

// Exposed directly here so inline onclick handlers work even for the
// statically-authored sections in index.html.
if (typeof window !== 'undefined') {
    window.toggleListFormat = toggleListFormat;
}
    ...
    sectionDiv.innerHTML = `
        <div class="section-header-editable">
            ...
            <div class="section-header-actions">
                <button class="btn-clear-section" onclick="window.clearSection('${inputId}', '${headingId}', '${sectionTitle}')" title="${t('additionalInfo.clearTitle')}" aria-label="${t('additionalInfo.clearTitle')}"><svg viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2L5 6"/><path d="M10 11v6"/><path d="M14 11v6"/><path d="M9 6V4a1 1 0 0 1 1-1h4a1 1 0 0 1 1 1v2"/></svg></button>
                <button class="btn-rename" onclick="window.toggleEditHeading('${headingId}')" title="${t('additionalInfo.renameTitle')}" aria-label="${t('additionalInfo.renameTitle')}"><svg viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><line x1="12" y1="4" x2="12" y2="20"/><path d="M8 4h8"/><path d="M8 20h8"/></svg></button>
                <button class="btn-list-format" onclick="window.toggleListFormat('${inputId}','bullet')" title="${t('additionalInfo.bullets')}" aria-label="${t('additionalInfo.bullets')}"><svg viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" aria-hidden="true"><line x1="9" y1="6" x2="20" y2="6"/><line x1="9" y1="12" x2="20" y2="12"/><line x1="9" y1="18" x2="20" y2="18"/><circle cx="4.5" cy="6" r="1.6" fill="currentColor" stroke="none"/><circle cx="4.5" cy="12" r="1.6" fill="currentColor" stroke="none"/><circle cx="4.5" cy="18" r="1.6" fill="currentColor" stroke="none"/></svg></button>
                <button class="btn-list-format" onclick="window.toggleListFormat('${inputId}','number')" title="${t('additionalInfo.numbers')}" aria-label="${t('additionalInfo.numbers')}"><svg viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><line x1="10" y1="6" x2="21" y2="6"/><line x1="10" y1="12" x2="21" y2="12"/><line x1="10" y1="18" x2="21" y2="18"/><path d="M3.5 5.2 5 4.4V10"/><path d="M3.6 10h2.8"/><path d="M3.2 15.4a1.4 1.4 0 1 1 2.4 1c-.5.6-2.2 1.7-2.4 2.6h2.8"/></svg></button>
...
export async function exportToPDF() {
    ...
    try {
        ...
        // ── Spacing constants ─────────────────────────────────────
        // All spacing in mm. These replace the scattered magic numbers
        // that caused overlapping text and tight cells.
        const margin      = 10;
        const cellPadX    = 3;     // horizontal padding inside every cell
        const cellPadTop  = 5;     // top padding so text never touches border
        const cellPadBot  = 4;     // extra bottom clearance per cell
        const lineH12     = 5.5;   // line pitch for 12pt text  (12pt ≈ 4.2mm glyph)
        const lineH14     = 6.5;   // line pitch for 14pt text  (14pt ≈ 4.9mm glyph)
        const minRowH     = 20;    // minimum task row height (mm)
        const minHdrH     = 16;    // minimum duty-header height (mm)
        ...
        const rightColW = pageWidth / 2 - margin - 10 - 10;  // available width for right column
        ...
        const innerW     = colWidth - (cellPadX * 2);  // text wraps within this width
        let dutyIndex = 0;
        ...
        while (dutyIndex < duties.length) {
            ...
            // Second pass: draw with uniform height.
            for (let col = 0; col < dtr; col++) {
            ...
            const maxTasks = Math.max(
                ...duties.slice(dutyIndex, dutyIndex + dtr).map(d => d.tasks.length)
            );
            for (let taskRow = 0; taskRow < maxTasks; taskRow++) {
                ...
                for (let col = 0; col < dtr; col++) {
                    ...
                    if (task) {
                        ...
                        const totalLines = 1 + taskLines.length;   // label + wrapped task
                        const h = cellPadTop + totalLines * lineH12 + cellPadBot;
        ...
        info.custom.forEach(sec => {
            ...
            div.innerHTML = `
                <div class="section-header-editable">
                    ...
                    <div class="section-header-actions">
                        <button class="btn-clear-section" onclick="window.clearSection('${inputId}','${headingId}','${sec.heading || ''}')" title="Clear section" aria-label="Clear section"><svg viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2L5 6"/><path d="M10 11v6"/><path d="M14 11v6"/><path d="M9 6V4a1 1 0 0 1 1-1h4a1 1 0 0 1 1 1v2"/></svg></button>
                        <button class="btn-rename" onclick="window.toggleEditHeading('${headingId}')" title="Rename section" aria-label="Rename section"><svg viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><line x1="12" y1="4" x2="12" y2="20"/><path d="M8 4h8"/><path d="M8 20h8"/></svg></button>
                        <button class="btn-list-format" onclick="window.toggleListFormat('${inputId}','bullet')" title="Bulleted list" aria-label="Bulleted list"><svg viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" aria-hidden="true"><line x1="9" y1="6" x2="20" y2="6"/><line x1="9" y1="12" x2="20" y2="12"/><line x1="9" y1="18" x2="20" y2="18"/><circle cx="4.5" cy="6" r="1.6" fill="currentColor" stroke="none"/><circle cx="4.5" cy="12" r="1.6" fill="currentColor" stroke="none"/><circle cx="4.5" cy="18" r="1.6" fill="currentColor" stroke="none"/></svg></button>
                        <button class="btn-list-format" onclick="window.toggleListFormat('${inputId}','number')" title="Numbered list" aria-label="Numbered list"><svg viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><line x1="10" y1="6" x2="21" y2="6"/><line x1="10" y1="12" x2="21" y2="12"/><line x1="10" y1="18" x2="21" y2="18"/><path d="M3.5 5.2 5 4.4V10"/><path d="M3.6 10h2.8"/><path d="M3.2 15.4a1.4 1.4 0 1 1 2.4 1c-.5.6-2.2 1.7-2.4 2.6h2.8"/></svg></button>
